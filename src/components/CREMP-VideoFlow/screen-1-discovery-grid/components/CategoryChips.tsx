...
export default function CategoryChips({ categories, active, onChange }: CategoryChipsProps) {
  return (
    <div className="flex items-center gap-2 px-3 pb-3 pt-2 overflow-x-auto touch-pan-x [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      {categories.map((cat) => {
        ...
        return (
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className={`inline-flex items-center justify-center gap-1.5 px-3.5 py-[7px] rounded-full shrink-0 text-[12px] transition-all duration-300 ${
              isActive ? 'vf-pill-luxury-active font-semibold' : 'vf-pill-luxury-inactive font-medium'
            }`}
