...
export default function SidebarNavigation({ activePage, onNavigate, userType, isDesktop, mobileOpen = false, onMobileClose }: SidebarNavigationProps) {
  ...
  if (isDesktop) {
    return (
      <nav className="w-52 shrink-0 bg-white border-r border-black/[0.03] flex flex-col py-3 overflow-y-auto">
        {items.map(({ id, label, Icon }) => (
          <button
            key={id}
            onClick={() => onNavigate(id)}
            className={`flex items-center gap-3 px-4 py-2.5 text-left text-sm transition-all duration-200 ${
              activePage === id
                ? 'bg-gradient-to-r from-[#d4af37]/10 to-transparent text-[#0a1128] border-r-2 border-[#d4af37] font-medium'
                : 'font-light text-[#637089] hover:bg-[#fafafb] hover:text-[#0a1128]'
            }`}
          >
          ...
          {items.map(({ id, label, Icon }) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-5 py-3 text-left text-sm transition-all duration-150 ${
                activePage === id
                  ? 'bg-gradient-to-r from-[#d4af37]/10 to-transparent text-[#0a1128] border-l-2 border-[#d4af37] font-medium pl-[18px]'
                  : 'font-light text-[#637089] hover:bg-[#fafafb] hover:text-[#0a1128] border-l-2 border-transparent pl-[18px]'
              }`}
            >
