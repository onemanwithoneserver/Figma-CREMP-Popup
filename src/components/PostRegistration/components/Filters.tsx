...
function ModernDropdown({ icon: Icon, placeholder, value, options, onChange, align = 'left' }: ModernDropdownProps) {
  const [isOpen, setIsOpen] = useState(false);
  const selectedLabel = options.find((o) => o.value === value)?.label;

  return (
    <div className={`relative w-full md:w-auto ${isOpen ? 'z-50' : 'z-10'}`}>
      <button
        type="button"
        onClick={() => setIsOpen(!isOpen)}
        className={`w-full flex items-center justify-between gap-1 px-2 md:px-3 py-2 bg-white border rounded-[4px] text-[11px] md:text-[13px] transition-all duration-300 font-medium shadow-[0_2px_8px_rgba(0,0,0,0.02)] outline-none h-[38px] ${
      ...
      {isOpen && (
        <>
          ...
          <div className={`absolute top-[calc(100%+6px)] ${align === 'right' ? 'right-0' : 'left-0'} min-w-[150px] md:min-w-[180px] bg-white border border-black/5 rounded-[4px] shadow-[0_8px_24px_rgba(0,0,0,0.08)] py-1.5 z-50 animate-in fade-in slide-in-from-top-2 duration-200`}>
            <button
              onClick={() => { onChange(''); setIsOpen(false); }}
              className={`w-full text-left px-4 py-2 text-[12px] md:text-[13px] font-medium transition-colors hover:bg-[#fafafb] hover:text-[#d4af37] ${
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col md:flex-row gap-2.5 md:gap-3 w-full">
        <div className="flex items-center gap-2.5 w-full md:flex-1">
          ...
          <div className="md:hidden shrink-0 flex items-center h-[38px]">
        ...
        <div className="hidden md:flex shrink-0 ml-auto items-center h-[38px]">
          <ToggleViewTabs view={view} onChange={setView} />
        </div>
      </div>
